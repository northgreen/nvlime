...
local function calc_keymaps_opts(args)
  local _let_2_ = pwin["get-screen-size"]()
  ...
  local _let_4_ = pwin["get-screen-pos"]()
...
local function win_callback(winid)
  ...
  return nvim_create_autocmd("WinLeave", {callback = _7_, once = true})
end
local function open_keymaps_win(bufnr)
  ...
  do
    ...
    for _, map in ipairs(km.buffer.get()) do
      local val_25_ = format_keymap_line(map.mode, map.lhs, map.desc)
...
keymaps.toggle = function()
  ...
  local case_10_, case_11_ = pbuf["visible?"](bufnr)
  if ((case_10_ == true) and (nil ~= case_11_)) then
    local winid = case_11_
