...
local function jump_to_coord(backward_3f)
  local _let_6_ = ut["get-win-cursor"](0)
  local linenr = _let_6_[1]
  local col = _let_6_[2]
  local jump
  local function _7_(coord)
    if coord then
...
local function do_cur_presentation(func)
  if not vim.tbl_contains(vim.b.nvlime_conn.cb_data.contribs, "SWANK-PRESENTATIONS") then
    ...
    local _let_11_ = ut["get-win-cursor"](0)
    local linenr = _let_11_[1]
...
repl.add = function()
  ...
  km.buffer.normal(rm.normal.clear, _16_, "nvlime: Clear the REPL buffer")
  ...
  km.buffer.normal(rm.normal.next_result, _17_, "nvlime: Move the cursor to the next presented object")
  local function _18_()
    return jump_to_coord(true)
  end
  return km.buffer.normal(rm.normal.prev_result, _18_, "nvlime: Move the cursor to the next presented object")
end
return repl
