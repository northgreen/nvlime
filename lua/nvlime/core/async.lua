...
local function dispatch_msg(chan, json_obj)
    ...
    if msg_id then
      ...
      if CB then
        ...
        if not ok then
          ...
          nvim_err_writeln(("nvlime: callback failed: " .. tostring(err)))
        else
        end
      else
        logger:get():warn()(("dispatch: NO CALLBACK for msg-id=" .. tostring(msg_id)))
      end
    else
    end
  end
  local function chan_input_cb(chan_id, data, event)
    local chan0 = chan_registry[chan_id]
    if chan0 then
      local obj_list = {}
      local buffered = (chan0.recv_buffer or "")
      ...
      chan0.recv_buffer = buffered
      for _, json_obj0 in ipairs(obj_list) do
        dispatch_msg(chan0, json_obj0)
      end
      return nil
    else
      return nil
  ...
  async["ch-sendexpr"] = function(chan0, expr, callback)
    local real_chan = chan_registry[chan0.ch_id]
    if not real_chan then
      error(("async.ch-sendexpr: channel " .. chan0.ch_id .. " not in registry, connection may be closed"))
    else
    end
